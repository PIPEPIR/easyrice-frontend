...
const HistoryTable = () => {
  ...
  const handleDelete = useCallback(async () => {
    try {
      ...
      setInspections((prev) =>
        prev.filter(
          (inspection) => !selectedIDs.includes(inspection.inspectionID)
        )
      );
    ...
    return inspections.map((inspection) => (
      ...
      >
        ...
        <TableCell className="text-body2">
          {formatDate(inspection.createDate)}
        </TableCell>
        <TableCell className="text-body2 text-ellipsis">
          {inspection.inspectionID}
        </TableCell>
        <TableCell className="text-body2">{inspection.name}</TableCell>
        <TableCell className="text-body2">{inspection.standardName}</TableCell>
        <TableCell className="text-body2">{inspection.note || "-"}</TableCell>
      </TableRow>
    ));
  };
    ...
    <>
      {selected.length > 0 && (
        <div className="pb-6 flex items-center gap-2">
          <LoadingButton
            className="text-subtitle3 font-semibold"
            variant="outlined"
